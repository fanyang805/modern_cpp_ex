...
bool PngIoStrategy::Write(const std::string& file_name,
  ...
  for (int r = 0; r < data.rows; ++r) {
    for (int c = 0; c < data.cols; ++c) {
      ...
      image[r][c] = png::rgb_pixel(data.data[0][idx], data.data[1][idx],
                                   data.data[2][idx]);
    }
  }
...
ImageData PngIoStrategy::Read(const std::string& file_name) const {
  ...
  std::vector<uint8_t> red(image_data.rows * image_data.cols, 0);
  std::vector<uint8_t> green(image_data.rows * image_data.cols, 0);
  std::vector<uint8_t> blue(image_data.rows * image_data.cols, 0);
