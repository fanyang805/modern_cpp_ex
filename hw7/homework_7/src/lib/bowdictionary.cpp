...
void BowDictionary::set_max_iterations(const int &max_iterations) {
  ...
  if (!need_update(max_iterations, dict_size_, descriptors_, total_features_)) {
    return;
  }
...
void BowDictionary::set_size(const int &dict_size) {
  if (!need_update(max_iterations_, dict_size, descriptors_, total_features_)) {
    return;
  }
...
void BowDictionary::set_params(const int &max_iterations, const int &dict_size,
                               const std::vector<cv::Mat> &descriptors) {
  ...
  dictionary_ = kMeans(mat_data, dict_size, max_iterations);
